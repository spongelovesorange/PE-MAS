# Manage the importable submodules, functions, classes, and variables for PE-GPT
"""
@reference: PE-GPT: a New Paradigm for Power Electronics Design, by Fanfan Lin, Xinze Li, et al.
@code-author: Xinze Li, Fanfan Lin
@github: https://github.com/XinzeLee/PE-GPT

@reference:
    Following references are related to power electronics GPT (PE-GPT)
    1: PE-GPT: a New Paradigm for Power Electronics Design
        Authors: Fanfan Lin, Xinze Li (corresponding), Weihao Lei, Juan J. Rodriguez-Andina, Josep M. Guerrero, Changyun Wen, Xin Zhang, and Hao Ma
        Paper DOI: 10.1109/TIE.2024.3454408
"""

# import submodules
# [CRITICAL UPDATE] Commented out to prevent cascade loading of Torch/DeepLearning modules
# which cause instability in the Flyback agent.
# from . import design_workflow
# NOTE: Avoid importing GUI/Streamlit modules at package import time.
# They produce side effects when backend services import `core` transitively.
# from . import gui
# from . import knowledge
# from . import llm
# from . import model_zoo
# from . import optim
# from . import simulation
# from . import utils




__all__ = [
    # importable submodules
    # design_workflow,
    # gui,
    # knowledge,
    # llm,
    # model_zoo,
    # optim,
    # simulation,
    # utils,
    
    
    # importable files
    
    
    # importable functions
    
    
    # importable classes
    
    
    # importable variables
    # please directly import from the files #
    
    
    ]
